...
export async function generateMetadata() {
    return {
        title: 'गढ़वा न्यूज़ ब्यूरो | NR Global Agency',
        description: 'गढ़वा जिले की ताज़ा खबरें, विश्लेषण और ग्राउंड रिपोर्ट। NR Global Agency - निष्पक्ष पत्रकारिता का वैश्विक मानक।',
        openGraph: {
            images: ['/og-garhwa.png']
        }
    }
}

export default async function GarhwaPage({ searchParams }: { searchParams: Promise<{ page?: string }> }) {
    const { page } = await searchParams
    const currentPage = Number(page) || 1
    const offset = (currentPage - 1) * POSTS_PER_PAGE

    const [pgData, snArticles, snTotal] = await Promise.all([
        getNewsByDistrict('garhwa', POSTS_PER_PAGE, offset),
        client.fetch(
            `*[_type == "article" && district == "garhwa"] | order(publishedAt desc)[$offset...$limit] {
                _id, title, "slug": slug.current, excerpt, featureImage, publishedAt, author->{name}
            }`,
            { offset, limit: offset + POSTS_PER_PAGE }
        ),
        client.fetch(`count(*[_type == "article" && district == "garhwa"])`)
    ])
                ...
                <div className="bg-brand-navy pt-16 pb-24 px-4 relative overflow-hidden">
                    ...
                    <div className="container max-w-6xl mx-auto text-center relative z-10">
                        ...
                        <h1 className="text-4xl md:text-6xl lg:text-7xl font-black text-white font-serif leading-tight mb-6">
                            गढ़वा <span className="text-brand-gold italic">ब्यूरो</span>
                        </h1>
                            ...
                            <>
                                ...
                                {totalPages > 1 && (
                                    <div className="mt-20 pt-10 border-t border-gray-50">
                                        <Pagination currentPage={currentPage} totalPages={totalPages} basePath="/garhwa" />
                                    </div>
                                )}
                            </>
                        )}
