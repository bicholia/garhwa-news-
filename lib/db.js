...
import { normalizeText, scrubBrandNames, scrubSlug } from './safety';
...
async function getSanityFallback(query, params = {}, limit = 10) {
  try {
    ...
    return data.map(item => ({
      ...
      // Fix: Resolve Sanity image URL if possible
      image_url: item.image_url || (item.featureImage?.asset ? `https://cdn.sanity.io/images/${process.env.NEXT_PUBLIC_SANITY_PROJECT_ID}/${process.env.NEXT_PUBLIC_SANITY_DATASET}/${item.featureImage.asset._ref.replace('image-', '').replace('-jpg', '.jpg').replace('-png', '.png').replace('-webp', '.webp')}` : null),
...
export async function isTitleExists(title) {
  ...
  try {
    ...
    // Rough check: We'll fetch titles and compare normalized versions
    const { rows } = await sql`SELECT title FROM news ORDER BY published_at DESC LIMIT 50`;
    return rows.some(r => normalizeText(r.title) === norm);
  } catch (error) {
    return false;
  }
}
  ...
  if (!hasPostgres()) {
       ...
       return {
         articles: articles.map(item => ({
           ...
           id: item._id,
           published_at: item.publishedAt,
           // Fix: Map Sanity image URL properly
           image_url: item.image_url || (item.featureImage?.asset ? `https://cdn.sanity.io/images/${process.env.NEXT_PUBLIC_SANITY_PROJECT_ID}/${process.env.NEXT_PUBLIC_SANITY_DATASET}/${item.featureImage.asset._ref.replace('image-', '').replace('-jpg', '.jpg').replace('-png', '.png').replace('-webp', '.webp')}` : null),
