  ...
  allRaw.forEach(item => {
    ...
    if (!seenSlugs.has(slug) && !seenTitles.has(title)) {
      merged.push({
        ...
        image_url: item.image_url || getSanityImageUrl(item.featureImage) || getSanityImageUrl(item.featureImage?.asset) || null,
        slug: scrubSlug(slug)
      });
      seenSlugs.add(slug);
...
export async function isNewsExists(slug) {
  try {
    ...
    const { client } = await import('./sanity');
    const sanityCount = await client.fetch(`count(*[_type == "article" && slug.current == $slug])`, { slug });
    if (sanityCount > 0) return true;

    // 2. Check Postgres if available
    if (hasPostgres()) {
      const { rows } = await sql`SELECT id FROM news WHERE slug = ${slug}`;
      if (rows.length > 0) return true;
    }
  } catch (error) {
    console.error('isNewsExists error:', error);
  }
  return false;
}
...
export async function isTitleExists(title) {
  try {
    ...
    // 1. Check Sanity first
    const { client } = await import('./sanity');
    // Using string matching in Sanity
    const sanityCount = await client.fetch(
      `count(*[_type == "article" && (title == $title || lower(title) == $lowerTitle || originalSource == $title)])`, 
      { title, lowerTitle: title.toLowerCase() }
    );
    if (sanityCount > 0) return true;
    ...
    if (hasPostgres()) {
      const { rows } = await sql`
          ...
          OR original_source ILIKE ${title}
          LIMIT 1
      `;
      if (rows.length > 0) return true;
    }
